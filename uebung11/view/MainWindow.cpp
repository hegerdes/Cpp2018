...
MainWindow::MainWindow(
    ...
    const std::string& end, int w, int h)
    : m_camera(Vector3f(0.0f, 0.0f, -700.0f), 0.05f, 5.0f) 
{
    ...
    // Calculate path
    PathPlanner searchPath(mapFile);
    list<Vector<float> >::iterator it;
    m_path = searchPath.getPath(m_actor->getPosition(), start, end);

    // Initialize route
    m_navRoute = NavigationRoute::Ptr(new asteroids::NavigationRoute(m_path));
    m_actor->setNavigationPath(m_path);
}
...
{
    if(m_actor && m_actor->hasMesh() && m_sdlWindow && m_sdlGlcontext && m_navRoute)
    {
        bool loop = true;
