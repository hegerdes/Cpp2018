...
LevelParser::LevelParser(const std::string& file, SpaceCraft::Ptr& spaceCraft, Skybox::Ptr& sky, AsteroidField::Ptr& af)
{
    ...
    BOOST_FOREACH(ptree::value_type& v, tree.get_child("level"))
    {
        if (v.first == "skybox")
        {
            ...
            sky = Skybox::Ptr(new Skybox(v.second.get<int>("<xmlattr>.width", 1024), names));
            
        }
        else if (v.first == "asteroids")
        {
            af = AsteroidField::Ptr(new AsteroidField(v.second.get<int>("count", 0), 
        ...
        {
            spaceCraft = SpaceCraft::Ptr(new SpaceCraft(path + v.second.get<std::string>("<xmlattr>.filename", "error"),
