...
void ReadOBJ::getMesh(TexturedMesh& texMesh)
{
	...
	map<string, int> matNames;
    map<int, MaterialFaceList*> matGroups;
    MaterialFaceList* currentMatList = 0;
		...
		while(in.good())
		{
			...
			else if(keyword == "usemtl")
			{
				...
				else
				{
                    ...
                    map<int, MaterialFaceList*>::iterator matIt = matGroups.find(it->second);
                    if(matIt == matGroups.end())
                    {
                        MaterialFaceList* l = new MaterialFaceList;
    ...
    float* vertex_buffer = new float[vertices.size()];
    ...
    float* normal_buffer = new float[normals.size()];
    ...
    float* texture_coord_buffer = new float[texcoords.size()];
    ...
    for(map<int, MaterialFaceList*>::iterator it = matGroups.begin(); 
        it != matGroups.end(); it++)
    {
